...
def projectProperties = [
	 ...
	 choice(
		name: 'ENV',
		choices: 'test\nyctest\nspttest\nmtest\nkxtest\ndemo\nwbl\nyc\ngz\nkx',
		description: '请选择部署到的环境:测试(万柏林)、测试(运城)、测试(省平台)、测试(移动端)、测试(市科协)、演示、万柏林、运城、贵州、科协'
	 ),
	 choice(
		name: 'YH',
		choices: '"',
...
{node('mypod') 
		...
		container('maven') {
		    stage('工单审批') {
				...
				sh 'ENV=$(echo ${ENV}) && curl \'https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=4d6e1917-549e-438a-bfb3-72914f927168\' -H \'Content-Type: application/json\' -d \'{"msgtype": "text","text": { "content": "Jenkins构建提醒\r\nJenkins Job:\'$JOB_NAME\'\r\n部署环境:\'$ENV\'\r\n部署项目:\'$Artifacts\'"}}\''
				sh 'echo 非运城线上环境可自由构建'
                sh 'if [ ${ENV} = yc ] || [ ${ENV} = wbl ];then curl -k "https://cmdb.hseduyun.net/auditapi/" -X POST -d "proname=${Artifacts}&proenv=${ENV}&action=reset"; echo 上线需要审批; echo 等待审核中...;c=0;while [ $c -lt 10 ]; do sleep 3;CODE=`wget -qO- --no-check-certificate "https://cmdb.hseduyun.net/auditapi/?proname=${Artifacts}&proenv=${ENV}"`;if [ ${CODE} -eq 1 ]; then echo ${ENV} ${Artifacts}工单审批已通过开始部署; echo break1;break;fi;if [ $c -eq 9 ]; then error echo 工单审批超时;fi; if [ ${CODE} -eq 0 ]; then error echo 工单已被拒绝;fi; c=`expr $c + 1`; done ;  fi'
		    }
			stage('svn代码检出') {
		...
		stage('k8s部署镜像') {
			container('kubectl') {
				...
				sh 'Artifacts=$(echo ${Artifacts})'
				sh 'echo 开始打印项目启动日志'
                sh 'PN=`kubectl --kubeconfig=/root/.kube/config get po -n ${ENV}|grep ${Artifacts}-deploy|head -1|awk \'{print \$1}\'`;wget -qO- "https://cmdb.hseduyun.net/k8slogapi/?ns=${ENV}&pod=${PN}&lines=300"|cat'
			}				
			}
		}
      	}
